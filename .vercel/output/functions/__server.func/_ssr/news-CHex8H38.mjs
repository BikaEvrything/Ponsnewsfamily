//#region node_modules/.nitro/vite/services/ssr/assets/news-CHex8H38.js
var CATEGORIES = [
	"All",
	"Wire",
	"Protocol",
...
function syntheticSpark(changeAbs, seed = 0) {
	const points = 28;
	const end = 100;
	const start = end / (1 + changeAbs / 100);
	return Array.from({ length: points }, (_, index) => {
		const t = index / 27;
		const wave = Math.sin((index + seed) * .55) * Math.max(1.2, Math.abs(changeAbs) * .12);
...
var FALLBACK_MARKETS = [
	...
	...row,
	kind: "token",
	spark: syntheticSpark(row.changeAbs, row.name.length)
}));
var FALLBACK_STOCKS = [
	{
		id: "hood",
...
function articleFromHeadline(input) {
	const blob = `${input.title} ${input.ticker ?? ""}`.toLowerCase();
	const feed = input.feed ? input.feed : blob.includes("pons") ? "pons" : blob.includes("robinhood") || blob.includes("hood") ? "robinhood" : "stocks";
	const published = input.publishedAt ? (/* @__PURE__ */ new Date(input.publishedAt * 1e3)).toLocaleString("en-US", {
		month: "long",
		day: "numeric",
		year: "numeric"
...
function articleFromMarket(row, fetchedAt = Date.now()) {
	const verb = row.changeAbs >= 0 ? "bids" : "offers";
	const isStock = row.kind === "stock";
	return {
		slug: `live-${row.id}`,
		title: isStock ? `PONS NEWS: ${row.ticker} ${verb} ${row.change}. Approved Pons pair.` : `PONS NEWS: ${row.name} ${row.ticker} ${verb} ${row.change} on the session.`,
		dek: isStock ? `${row.name} last ${row.price}. Pons V2 lets a launch pair against tokenized ${row.ticker}.` : `Last print ${row.price}. Market cap ${row.mcap}. Volume ${row.volume}.`,
...
function snapshotFromMarkets(markets, statusTitle, statusMessage, fetchedAt = Date.now(), stocks = FALLBACK_STOCKS) {
	const pons = markets.find((row) => row.id === "pons") ?? markets[0];
	const tape = [...stocks.map((row) => ({
		label: `${row.name} ${row.ticker} ${row.change}`,
		meta: row.price,
		topic: row.id === "hood" ? "robinhood" : "stocks"
	})), ...markets.map((row) => ({
		label: `${row.name} ${row.ticker} ${row.change}`,
		meta: row.price,
		topic: "pons"
...
var EMPTY_WIRE = snapshotFromMarkets(FALLBACK_MARKETS, "Degraded performance", "We are upgrading the backend ahead of a new rollout. Launches and market data may load slowly or read out of date.", Date.now(), FALLBACK_STOCKS);
var DESK_ARTICLES = [
	{
	...
	},
	{
		...
		source: "Pons V2 · docs",
		feed: "stocks",
		body: ["Pons V2 lets a creator set the paired asset at launch. Tokenized stocks on this board include HOOD, NVDA, AAPL, TSLA and the rest of the list.", "The pair carries into the locked pool at graduation."]
	},
	{
		...
		feed: "robinhood",
		body: ["Pons is a launchpad on Robinhood Chain.", "HOOD is the house name on this desk."]
	},
	{
...
function articleMatchesFeed(article, feed) {
...
//#endregion
export { FALLBACK_STOCKS as a, articleMatchesFeed as c, getDeskArticle as d, relatedArticles as f, timeAgo as h, FALLBACK_MARKETS as i, formatPct as l, syntheticSpark as m, DESK_ARTICLES as n, articleFromHeadline as o, snapshotFromMarkets as p, EMPTY_WIRE as r, articleFromMarket as s, CATEGORIES as t, formatUsd as u };
